...
Description: Autonomous robot with SLIDE-COMPLIANT PIPELINE.
             
PIPELINE STEPS:
1. Gaussian Blur (3x3 Kernel) - As per Slide 2
2. Grayscale Conversion
3. Edge Detection (Gradient Magnitude)
4. Feature Extraction (Edge Counting with Spatial Filtering)

CLASSIFICATION LOGIC:
- 0-1 Edges -> Wall (Background)
- 2-4 Edges -> Box (Left + Right sides detected)
- >4 Edges  -> Robot (Complex geometry)
"""
...
EDGE_LIMIT_WALL = 1        # 0-1 edges = Wall
EDGE_LIMIT_BOX = 8         # INCREASED: Allow up to 8 edges for a box (ignoring text noise)

# --- Edge Cooldown (The Text Fix) ---
# INCREASED: Ignore the next 30 pixels after finding an edge.
...
def get_gaussian_pixel(img, x, y, width):
    ...
    # Center*4 + Neighbors*1 / 8 (Simplified bit-shift logic)
...
def run_vision_pipeline(img_current, img_prev, width, height):
    ...
    for y in range(0, height, step):
        for x in range(0, width, step):
            ...
            # Simple Grayscale for Motion (Speed optimization: No blur needed for motion)
    ...
    for x in range(2, width - 6):
        ...
        # [PIPELINE STEP 1]: Get Gaussian Blurred Pixel (Current)
        b, g, r = get_gaussian_pixel(img_current, x, scan_y, width)
        gray = (r + g + b) // 3
        
        # [PIPELINE STEP 1]: Get Gaussian Blurred Pixel (Neighbor +4px)
        b2, g2, r2 = get_gaussian_pixel(img_current, x+4, scan_y, width)
        gray_next = (r2 + g2 + b2) // 3
        
        # [PIPELINE STEP 3]: Edge Gradient
        ...
        if diff > EDGE_SENSITIVITY:
            ...
            if x > 10 and x < (width - 10):
                ...
                # TRIGGER COOLDOWN
                # We found an edge (e.g. Left side of 'A'). 
                # Ignore the rest of 'A' and 'B' and 'C'.
                # We will likely wake up in time to catch the Right side of the box.
